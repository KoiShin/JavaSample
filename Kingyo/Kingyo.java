...
public class Kingyo extends Applet implements Runnable, MouseListener {
    ...
    double KingyoWidth[]  = new double[50];
    double KingyoHeight[] = new double [50];        // イメージのサイズ
    int AppletWidth, AppletHeight;                  // アプレットのサイズ

    Image WorkImage;                                // 作業用イメージ
    Graphics WorkGraphics;                          // 作業用グラフィックス

    double growthCondition[] = new double[50];

    double defaultKingyoWidth;
    double defaultKingyoHeight;

    int fullStomachCondition[] = new int[50];
    ...
    public void init() {
        ...
        defaultKingyoWidth = KingyoImage[0].getWidth(this) * 0.5;
        defaultKingyoHeight = KingyoImage[0].getHeight(this) * 0.5;
        for (int i = 0; i < Number; i++) {
            KingyoWidth[i]  = defaultKingyoWidth;        // 金魚の幅と高さ
            KingyoHeight[i] = defaultKingyoHeight;
        }

        for (int i = 0; i < Number; i++) {
            growthCondition[i] = 1.0;
            fullStomachCondition[i] = 0;
        }

        // 金魚の初期化
        for (int i = 0; i < Number; i++) {
            Direction[i] = (int)(Math.random() * 16);                        // 方向
            X[i] = (int)(Math.random() * (AppletWidth - KingyoWidth[i]));       // 位置
            Y[i] = (int)(Math.random() * (AppletHeight - KingyoHeight[i]));
        }
        // 餌の初期化
    ...
    void Swimming() {
        ...
        for (int i = 0; i < Number; i++) {
            ...
            // 場外に出ないようにチェック
            if (xp >= 0 && xp <= AppletWidth  - KingyoWidth[i] &&
                yp >= 0 && yp <= AppletHeight - KingyoHeight[i]) {
                X[i] = xp;
                Y[i] = yp;
            }

            WorkGraphics.drawImage(KingyoImage[Direction[i]], xp, yp,
                (int)KingyoWidth[i], (int)KingyoHeight[i], this);
            ...
            // 次の方向セット
            if (foodFlag == 0 || fullStomachCondition[i] > 0) {
                ...
                // 餌が金魚の画像内の場合，餌を削除
                if (X[i] <= FoodX[nearPoint] && FoodX[nearPoint] <= X[i] + KingyoWidth[i] &&
                    Y[i] <= FoodY[nearPoint] && FoodY[nearPoint] <= Y[i] + KingyoHeight[i]) {
                    Food[nearPoint] = 0;
                    growthCondition[i] += 0.05;
                    KingyoWidth[i]  = defaultKingyoWidth * growthCondition[i];
                    KingyoHeight[i] = defaultKingyoHeight * growthCondition[i];
                    fullStomachCondition[i] += 100;
                }
            }
        }
    }
    ...
    public void mousePressed(MouseEvent evt) {
        ...
        for (int i = 0; i < FoodNumber; i++) {
            if (Food[i] == 0) {
                Food[i] = 1;
                // 餌の位置  マウスをクリックした位置を中心に半径50の範囲にランダムにまく
