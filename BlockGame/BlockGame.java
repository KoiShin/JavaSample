...
public class BlockGame extends Applet
    ...
    Image BallImage, RacketImage;                    // ボール，ブロック，ラケットイメージ
    Image BlockImage[] = new Image[3];
    ...
    public void init() {
        ...
        BlockImage[0] = getImage(getCodeBase(), getParameter("block"));    // ブロック画像
        mt.addImage(BlockImage[0], 0);
        BlockImage[1] = getImage(getCodeBase(), getParameter("bonusBlock"));    // ブロック画像
        mt.addImage(BlockImage[1], 0);
        BlockImage[2] = getImage(getCodeBase(), getParameter("noBrokenBlock"));    // ブロック画像
        mt.addImage(BlockImage[2], 0);
        ...
        BlockWidth = BlockImage[0].getWidth(this);        // ブロックサイズ
        BlockHeight = BlockImage[1].getHeight(this);
    ...
    public void GameScreenDisplay() {
        ...
        for (int i = 0; i < BlockRow; i++) {           // 縦方向 行
            for (int j = 0; j < BlockColumn; j++) {    // 横方向 列
                ...
                WorkGraphics.drawImage(BlockImage[num],     // 作業グラフィックにブロック描画
                    BlockX + BlockWidth * j,
                    BlockY + BlockHeight * i, this);
                Block[i][j] = 1;                    // 配列にブロックセット
                BlockInfo[i][j] = num;
                switch (BlockInfo[i][j]) {
                    case 0:
                        expectedScore++;
                        break;
                    case 1:
            ...
            if (xw >= BlockX && xw < BlockX + BlockWidth * BlockColumn
                ...
                if (Block[yp][xp] == 1) {                // ブロックがある場合
                    ...
                    if (!noBroken) {
                        ...
                        // setXORMode(Color.white)の設定によって，上書きで消す
                        WorkGraphics.drawImage(BlockImage[BlockInfo[yp][xp]],    // グラフィック上でブロックを消す
