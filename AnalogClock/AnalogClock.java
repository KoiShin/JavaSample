...
public class AnalogClock extends Applet implements Runnable {
	...
	void MakeHand(int cx, int cy, double kakudo, double nagasa, double hutosa, Color color) {
	    int px[] = new int[4];
	    int py[] = new int[4];
	...
	void DispTime( ) {
		...
		MakeHand(CenterX, CenterY, hour * 30 + minute / 60.0 * 30, Radius * 0.8, 1.2, Color.black);
		MakeHand(CenterX, CenterY, minute * 6.0, Radius * 0.9, 0.7, Color.gray);

		// 秒の針描画
		double RD_ = second * 6 * Math.PI / 180;						// 秒の角度をラジアンに変換
		int sx = CenterX + (int)(Math.sin(RD_) * Radius * 0.9);	// 秒の針の先端の位置
		int sy = CenterY - (int)(Math.cos(RD_) * Radius * 0.9);
